...
class LeaderboardTable extends Component {
  ...
  componentDidMount() {
    fetch(process.env.REACT_APP_API_URL + "/api/games/currLeaderboard")
      .then((res) => res.json())
      .then((results) => {
        if (results.success) {
      ...
      .catch((err) =>
        console.log("Problem Loading Current Game Leaderboard", err)
      );
  }
  ...
  componentDidUpdate() {
    if (!this.state.lbLoaded && this.state.currGame) {
      ...
      for (let i = 0; i < this.state.currGame.length; i++) {
        ...
        this.state.currResults.push(
          <Player player={this.state.currGame[i]} rank={i + 1} />
        );
      }
      this.setState({
        lbLoaded: true,
        ...
        <div className="container">
          <div className="row">
            <Table striped bordered hover id="leaderboard" variant="dark">
              <Header />
              <tbody>{this.state.currResults}</tbody>
            </Table>
          </div>
        </div>
      );
