...
class GameCreation extends React.Component {
  ...
  async handleImageSubmit (e) {
    ...
    await axios.post(process.env.REACT_APP_API_URL + '/api/games/upload', formData, {
  ...
  handleSubmit (e) {
    ...
    // Call fetch function.
    fetch(process.env.REACT_APP_API_URL + '/api/games/createGame', {
      method: 'POST',
  ...
  componentDidMount () {
    ...
    if (storage && storage.token) {
      const { token } = storage
      fetch(process.env.REACT_APP_API_URL + '/api/account/getAcct/?acct=' + token)
        .then(res => res.json())
            ...
            {this.state.gamePhotoUploaded ?
              <div className='row'>
                <div className='col-6 m-auto'>
                  ...
                  <img alt='gamePhoto' style={{ width: '50%' }} src={this.state.gamePhotoUploaded.filePath} />
                </div>
              </div>
              : null}
          </div>
