...
class PlayerEntry extends React.Component {
  ...
  handleDelete () {
    //e.preventDefault();
    fetch(process.env.REACT_APP_API_URL + '/api/account/deleteAccount', {
      method: 'DELETE',
  ...
  handleUpdate () {
    ...
    if (this.props.player.acctType ==='player') {
      ...
      fetch(process.env.REACT_APP_API_URL + '/api/account/updatePlayerAcc', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        username: this.props.player.username
    ...
    .catch((err) => console.log('An error occured updating player the account type', err))
      ...
      fetch(process.env.REACT_APP_API_URL + '/api/account/updateAdminAcc', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        username: this.props.player.username
