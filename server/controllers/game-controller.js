...
export const upload = async (req, res) => {
  ...
  file.mv(
    `../../runtime-terror/client/public/images/uploads/${file.name}`,
    (err) => {
      if (err) {
...
export const currLeaderboard = async (req, res) => {
  const results = await PlayerStats.find().sort({
    kills: "desc",
    deaths: "asc",
    remaining_lives: "desc",
  });
...
export const gameEndTransfer = async (req, res) => {
  ...
  await GameDesc.findOneAndUpdate(
    { game_id: game_id },
    { ended: true },
    (err, game) => {
      if (err) {
...
export const currentGames = async (req, res, next) => {
  ...
  await GameDesc.find({
    ended: false,
    registration_deadline: { $lte: Date.now() },
  })
    .lean()
    .then((games) => {
      for (let i = 0; i < games.length; i++) {
